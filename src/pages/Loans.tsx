  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-xl font-bold text-surface-900 dark:text-surface-100">Loans</h1>
        <Link to="/blockchain" className="text-sm text-primary-600 dark:text-primary-400 hover:underline">
          On-chain
        </Link>
      </div>
          ...
          <>
            ...
            {eligibility.attestation && (
              <div className="mt-3 rounded-lg border border-surface-100 dark:border-surface-800 p-3 text-xs text-surface-600 dark:text-surface-400 space-y-1">
                ...
                <p className="font-mono">{shortHash(eligibility.attestation.txHash)}</p>
              </div>
            )}
              ...
              <li key={loan.id} className="rounded-lg border border-surface-100 dark:border-surface-800 p-3 text-sm">
                ...
                {loan.createTxHash && (
                  <div className="mt-2 text-xs text-surface-500 font-mono">
                    <p>chain #{loan.onChainLoanId} · {shortHash(loan.createTxHash)}</p>
                  </div>
                )}
              </li>
            ))}
